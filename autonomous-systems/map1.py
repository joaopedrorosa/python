...
#a primeira vez que se chama a func o i,j sao as coords da root	
	...
	if ((int(Map[i+1][j-1]) == 100) & (int(Occupancy_grid[i+1][j-1]) == 0)):
		...
		j = j-1
		flag = 1
		line_points = line_points + (vert_line(Map, Occupancy_grid,line_points,i,j),)
		return		
	if ((int(Map[i+1][j]) == 100) & (int(Occupancy_grid[i+1][j]) == 0)):
	...
	if flag != 1:	
		if int(Map[i+2][j-1]) == 100 & int(Occupancy_grid[i+2][j-1]) == 0:
			...
			line_points = line_points + (vert_line(Map, Occupancy_grid,line_points,i,j),)	
			return
		if int(Map[i+2][j]) == 100 & int(Occupancy_grid[i+2][j]) == 0:
...
if __name__ == "__main__":
	...
	while end != 2:
		...
		line_points = vert_line(x,Occupancy_grid,(line_points,),root[0],root[1])	
